...
async function setConfig(config: IConfig) {
  ...
  broadcastToAllWindows("config:change", config);
...
async function setOTAConfig(otaConfig: IOTAConfigPayload) {
  ...
  const config = await getConfig();
  broadcastToAllWindows("config:change", globalConfig);
  globalConfig = {
    ...defaultConfig,
    ...otaDefaultConfig,
    ...config,
...
async function fetchAuthoritativeConfig() {
  ...
  for (const hostname of config.authoritativeHostnames) {
    const url = new URL(`/api/ota-config/${platform}/${version}`, hostname);
    try {
...
function registerConfigIPCHandlers() {
  ...
  ipcMain.handle("config:reset", handleConfigReset);
  ipcMain.handle("config:open", handleConfigOpen);
  ipcMain.handle("config:updateOTA", fetchAuthoritativeConfig);

  return function () {
    ipcMain.removeHandler("config:get");
    ipcMain.removeHandler("config:set");
    ipcMain.removeHandler("config:reset");
    ipcMain.removeHandler("config:open");
  };
}
